...
export class PerfilComponent implements OnInit {
  ...
  loadProfile(): void {
    ...
    this.perfilService.getProfile().subscribe({
      ...
      error: (err) => {
        console.error('Error al cargar el perfil de administrador:', err);
  ...
  onDeleteAccount(): void {
    if (
      confirm(
        '¿Está seguro que desea eliminar su cuenta de administrador? Esta acción no se puede deshacer.'
      )
    ) {
      this.perfilService.deleteProfile().subscribe({
        ...
        error: (err) => {
          console.error('Error al eliminar la cuenta de administrador:', err);

          // Manejo específico del error 403
          if (err.status === 403) {
            let errorMsg = 'No tiene permisos para eliminar la cuenta de administrador.\n';
            errorMsg += 'Posibles causas:\n';
            ...
            alert('Error al eliminar la cuenta de administrador. Por favor, inténtelo más tarde.');
          }
        },
      });
    }
