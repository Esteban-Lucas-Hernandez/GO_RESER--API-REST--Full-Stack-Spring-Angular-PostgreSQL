...
public class SuperAdminPerfilController {
    ...
    @GetMapping
    public ResponseEntity<UsuarioDTO> getSuperAdminProfile() {
        try {
    ...
    @PutMapping
    public ResponseEntity<UsuarioDTO> updateSuperAdminProfile(@RequestBody ActualizarPerfilDTO actualizarPerfilDTO) {
        try {
            Usuario usuario = securityService.getCurrentUser();
            if (usuario == null) {
                return ResponseEntity.notFound().build();
